...
class BoundedAStar {
 public:
  BoundedAStar(Environment& environment, const size_t max_expansions)
      : m_env(environment), m_max_expansions(max_expansions) {}
  ...
  bool search(const State& startState,
    ...
    for (size_t exp = 0; exp < m_max_expansions && !openSet.empty(); ++exp) {
